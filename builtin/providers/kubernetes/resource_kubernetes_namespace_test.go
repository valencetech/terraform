		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					testAccCheckNamespaceAnnotations(&conf, map[string]string{"TestAnnotationOne": "one", "TestAnnotationTwo": "two"}),
					...
					testAccCheckNamespaceLabels(&conf, map[string]string{"TestLabelOne": "one", "TestLabelTwo": "two", "TestLabelThree": "three"}),
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					testAccCheckNamespaceAnnotations(&conf, map[string]string{"TestAnnotationOne": "one", "Different": "1234"}),
					resource.TestCheckResourceAttr("kubernetes_namespace.test", "metadata.0.labels.%", "2"),
					resource.TestCheckResourceAttr("kubernetes_namespace.test", "metadata.0.labels.TestLabelOne", "one"),
					resource.TestCheckResourceAttr("kubernetes_namespace.test", "metadata.0.labels.TestLabelThree", "three"),
					testAccCheckNamespaceLabels(&conf, map[string]string{"TestLabelOne": "one", "TestLabelThree": "three"}),
					resource.TestCheckResourceAttr("kubernetes_namespace.test", "metadata.0.name", nsName),
					resource.TestCheckResourceAttrSet("kubernetes_namespace.test", "metadata.0.generation"),
					resource.TestCheckResourceAttrSet("kubernetes_namespace.test", "metadata.0.resource_version"),
					resource.TestCheckResourceAttrSet("kubernetes_namespace.test", "metadata.0.self_link"),
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					testAccCheckNamespaceAnnotations(&conf, map[string]string{}),
					resource.TestCheckResourceAttr("kubernetes_namespace.test", "metadata.0.labels.%", "0"),
					testAccCheckNamespaceLabels(&conf, map[string]string{}),
					resource.TestCheckResourceAttr("kubernetes_namespace.test", "metadata.0.name", nsName),
					resource.TestCheckResourceAttrSet("kubernetes_namespace.test", "metadata.0.generation"),
					resource.TestCheckResourceAttrSet("kubernetes_namespace.test", "metadata.0.resource_version"),
					resource.TestCheckResourceAttrSet("kubernetes_namespace.test", "metadata.0.self_link"),
...
func testAccCheckNamespaceAnnotations(ns *api.Namespace, expected map[string]string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if len(expected) == 0 && len(ns.Annotations) == 0 {
			return nil
		}
		if !reflect.DeepEqual(ns.Annotations, expected) {
			return fmt.Errorf("%s annotations don't match.\nExpected: %q\nGiven: %q",
				ns.Name, expected, ns.Annotations)
		}
		return nil
	}
}

func testAccCheckNamespaceLabels(ns *api.Namespace, expected map[string]string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if len(expected) == 0 && len(ns.Labels) == 0 {
			return nil
		}
		if !reflect.DeepEqual(ns.Labels, expected) {
			return fmt.Errorf("%s labels don't match.\nExpected: %q\nGiven: %q",
				ns.Name, expected, ns.Labels)
		}
		return nil
	}
}
